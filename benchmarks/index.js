...
var bitstashd = require('../').services.Bitcoin({
  node: {
    ...
    function(next) {
      ...
      function bitcoindGetBlockNative(deffered) {
        ...
        var hash = fixtureData.blockHashes[c];
          bitstashd.getBlock(hash, function(err, block) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function bitcoinGetTransactionNative(deffered) {
        ...
        var hash = fixtureData.txHashes[c];
          bitstashd.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      suite.add('bitstashd getblock (native)', bitcoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('bitstashd getblock (json rpc)', bitcoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('bitstashd gettransaction (native)', bitcoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('bitstashd gettransaction (json rpc)', bitcoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
