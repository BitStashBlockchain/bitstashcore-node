...
var BITSTASHRPC = require('bitstashd-rpc');
...
/**
 * Provides a friendly event driven API to bitstashd in Node.js. Manages starting and
 * stopping bitstashd as a child process for application support, as well as connecting
 * to multiple bitstashd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function BITSTASH(options) {
  if (!(this instanceof BITSTASH)) {
    return new BITSTASH(options);
  }
...
util.inherits(BITSTASH, Service);

BITSTASH.dependencies = [];

BITSTASH.DEFAULT_MAX_TXIDS = 1000;
BITSTASH.DEFAULT_MAX_HISTORY = 50;
BITSTASH.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
BITSTASH.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
BITSTASH.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
BITSTASH.DEFAULT_SPAWN_RESTART_TIME = 5000;
BITSTASH.DEFAULT_SPAWN_STOP_TIME = 10000;
BITSTASH.DEFAULT_TRY_ALL_INTERVAL = 1000;
BITSTASH.DEFAULT_REINDEX_INTERVAL = 10000;
BITSTASH.DEFAULT_START_RETRY_INTERVAL = 5000;
BITSTASH.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
BITSTASH.DEFAULT_TRANSACTION_CONCURRENCY = 5;
BITSTASH.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcbind: '127.0.0.1',
  rpcuser: 'bitstash',
  rpcpassword: 'bitstashpassword',
  uacomment: 'bitcore'
};

BITSTASH.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || BITSTASH.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || BITSTASH.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || BITSTASH.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || BITSTASH.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || BITSTASH.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || BITSTASH.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || BITSTASH.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || BITSTASH.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || BITSTASH.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || BITSTASH.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

BITSTASH.prototype._initCaches = function() {
...
BITSTASH.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
BITSTASH.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
BITSTASH.prototype.getPublishEvents = function() {
  return [
    {
...
BITSTASH.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'bitstashd/' + name, 'total:', this.subscriptions[name].length);
};

BITSTASH.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bitstashd/' + name, 'total:', this.subscriptions[name].length);
};


BITSTASH.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
BITSTASH.prototype.unsubscribeBalance = function(emitter, addresses) {
...
BITSTASH.prototype.unsubscribeBalanceAll = function(emitter) {
...
BITSTASH.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'bitstashd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

BITSTASH.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'bitstashd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
BITSTASH.prototype.unsubscribeAddressAll = function(emitter) {
...
BITSTASH.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = BITSTASH.DEFAULT_CONFIG_SETTINGS;
...
BITSTASH.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
BITSTASH.prototype._expandRelativeDatadir = function() {
...
BITSTASH.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in bitstashd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in bitstashd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in bitstashd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './bitstash.conf');
  if (spawnOptions.configdir != null) {
    configPath = path.resolve(spawnOptions.configdir, './bitstash.conf');
  }

  log.info('Using bitstash config file:', configPath);
...
BITSTASH.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of bitstashcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitstashcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitstashcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitstashd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from bitstashd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from bitstashd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitstashd is undergoing a reindex. ' +
             ...
             'of bitstashcore-node services will start.');
    node._reindex = true;
  }
};

BITSTASH.prototype._resetCaches = function() {
...
BITSTASH.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
BITSTASH.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

BITSTASH.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('BITSTASH Daemon Ready');
          callback();
        });
      });
...
BITSTASH.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 4889
  };
...
BITSTASH.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/bitstash.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/bitstash.conf';
    }
  }
  return networkPath;
};

BITSTASH.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
BITSTASH.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('bitstashd/hashblock', message.toString('hex'));
    }
  }

};

BITSTASH.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
BITSTASH.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('BITSTASH Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
BITSTASH.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
BITSTASH.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
BITSTASH.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
BITSTASH.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('bitstashd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
BITSTASH.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('bitstashd/rawtransaction', message.toString('hex'));
    }
...
BITSTASH.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || BITSTASH.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

BITSTASH.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
BITSTASH.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
BITSTASH.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('BITSTASH Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || BITSTASH.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

BITSTASH.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
BITSTASH.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/bitstashd.pid';
...
BITSTASH.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting bitstash process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('BITSTASH process unexpectedly exited with code:', code);
        log.warn('Restarting bitstash child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('BITSTASH process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn bitstashd.'));
      }
...
BITSTASH.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to bitstashd.'));
    }
 ...
 */
BITSTASH.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('BITSTASH configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.syncPercentage = function(callback) {
  var self = this;
...
BITSTASH.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, bitstashcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
BITSTASH.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
BITSTASH.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
BITSTASH.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
BITSTASH.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in bitstashcore is most recent to less recent
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
BITSTASH.prototype._getConfirmationsDetail = function(transaction) {
...
BITSTASH.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
BITSTASH.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
BITSTASH.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A bitstash transaction id
 * @param {Function} callback
 */
BITSTASH.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
BITSTASH.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
BITSTASH.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
BITSTASH.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
BITSTASH.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a BITSTASHcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
BITSTASH.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getInfo = function(callback) {
...
BITSTASH.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('bitstashd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('bitstashd process did not exit'));
      }
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
BITSTASH.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
BITSTASH.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
BITSTASH.prototype.getStakingInfo = function(callback) {
    var self = this,
