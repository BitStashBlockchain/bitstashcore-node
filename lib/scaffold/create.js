...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'bitstash-lib': 'git://github.com/BitStashBlockchain/bitstash-lib.git#master',
    'bitstashcore-node': 'git://github.com/BitStashBlockchain/bitstashcore-node.git'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bitstashcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bitstashcore-node directory and configuration
