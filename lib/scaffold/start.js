...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitstashd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and bitstashcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed bitstashcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bitstashcore-node.json');
  ...
  node.on('ready', function() {
    log.info('BITSTASHcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in bitstashcore-node services directory
    ...
    if (servicePackage.bitstashcoreNode) {
      serviceModule = service.name + '/' + servicePackage.bitstashcoreNode;
    }
